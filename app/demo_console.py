...
from .registry_watch import build_registry_watch_demo_report
...
def render_demo_console() -> str:
    ...
    registry_watch = build_registry_watch_demo_report()
    registry_watch_latest = registry_watch["latest_run"]
    registry_action = registry_watch_latest["action_candidate"]
...
<body>
  <div class="shell">
    ...
    <main><header class="topbar"><div class="office">Vice CEO · Business Autonomy</div><nav aria-label="Page sections"><a href="#review">Review</a><a href="#evidence">Evidence</a></nav></header><div class="document">
      <article class="content" id="review"><a class="back" href="/demo/judge-flow">&larr; View the five-act proof flow</a><h1>From signal<br>to a <em>business decision.</em></h1><p class="lede">Vice CEO watches the business signals you approve and does the research in the background. When something needs attention, it gives you a clear, evidence-backed next step while you stay in control of any real-world action.</p><dl class="case-meta"><dt>Product</dt><dd>Proof-carrying business autonomy</dd><dt>Demo workflow</dt><dd>Synthetic support escalation</dd><dt>Live deployment example</dt><dd>Westover EPR Registry Change Watch</dd><dt>Authority</dt><dd class="ok">No external business tools</dd><dt>Release state</dt><dd>{escape(deployment_status)} · provider {escape(provider_status).lower()}</dd></dl>
      <div class="review-grid"><section class="document-section"><h2>1. Business decision</h2><p>Review a fixed, source-backed operational case before allowing its only permitted transition: a simulated outcome with no customer contact, persistence, or external effect.</p><h2>Why it is reusable</h2><p>The same evidence, warrant, and replay pattern can support compliance, operations, support, procurement, and other high-stakes workflows. Westover EPR is the first live registry-monitoring deployment, not the product boundary.</p><h2>Proposed next step</h2><ol><li>Inspect the proof bundle and warrant.</li><li>Compare the recorded alternatives.</li><li>Approve or retain the fixed synthetic simulation for review.</li></ol></section><section class="document-section"><h2>2. Review standard</h2><p>Every recommendation must be traceable, warranted, replayable, and bounded before an effect could occur. This screen demonstrates the product standard without granting real-world authority.</p><h2>3. Reviewer checks</h2><ul><li>Evidence chain is locally verified: {escape(verified)}.</li><li>{verification.safety_probe_count} adversarial safety probes are recorded.</li><li>{verification.evaluation_case_count} evaluation cases are included.</li><li>Local evaluation score: {verification.evaluation_score:.0%}.</li></ul></section></div>
      <section class="document-section" id="evidence"><h2>Evidence trail</h2><div class="proof-list"><a class="proof-row" href="/demo/proof-bundle"><span class="proof-index">01</span><span><strong>Integrity-linked proof bundle</strong><small>{escape(proof_bundle.bundle_id)}</small></span><span class="arrow">&nearr;</span></a><a class="proof-row" href="/demo/action-warrant-dossier"><span class="proof-index">02</span><span><strong>One-use Action Warrant</strong><small>First use: {escape(warrant_dossier.first_use_state)} · external effect: false</small></span><span class="arrow">&nearr;</span></a><a class="proof-row" href="/demo/time-machine-dossier"><span class="proof-index">03</span><span><strong>Replay record</strong><small>{escape(time_machine_dossier.replay_status)}</small></span><span class="arrow">&nearr;</span></a><a class="proof-row" href="/demo/registry-watch"><span class="proof-index">04</span><span><strong>Live deployment: EPR Change Watch</strong><small>Westover EPR example · fixture change: {escape(str(registry_watch_latest["status"]))} · {escape(str(registry_action["action_type"]).replace("_", " "))}</small></span><span class="arrow">&nearr;</span></a></div></section>
  ...
  <section class="approval-result" id="approval-result" aria-live="polite" aria-atomic="true"><span class="eyebrow">Synthetic decision</span><strong id="approval-title">Decision required</strong><p id="approval-message">Choose an option to resolve the fixed local demonstration.</p><small id="approval-details" hidden></small></section>
  <script>
    const result = document.getElementById("approval-result");
    const resultTitle = document.getElementById("approval-title");
    const resultMessage = document.getElementById("approval-message");
    const resultDetails = document.getElementById("approval-details");
    const showResult = (state, title, message, details = "") => {{
      result.className = `approval-result visible`;
      result.dataset.state = state;
      resultTitle.textContent = title;
      resultMessage.textContent = message;
      resultDetails.textContent = details;
      resultDetails.hidden = !details;
    }};
    document.querySelectorAll("[data-demo-approval]").forEach((button) => {{
      button.addEventListener("click", async () => {{
        ...
        showResult("pending", "Resolving decision", "Running the fixed local demonstration…");
        try {{
          ...
          if (!response.ok || !payload.human_approval) throw new Error("demo_approval_unavailable");
          const approval = payload.human_approval;
          if (approval.simulation_executed) {{
            showResult("success", "Simulation approved", "A warrant-backed synthetic receipt was created. No external action occurred.", `Receipt ${'{'}approval.outcome_receipt_id{'}'} · Warrant ${'{'}approval.action_warrant_id{'}'}`);
          }} else {{
            showResult("review", "Kept in review", "No simulation was started. The case remains a local review fixture.");
          }}
        }} catch (error) {{
          showResult("error", "Decision unavailable", "No simulation was started. Try again after refreshing the page.");
        }} finally {{
