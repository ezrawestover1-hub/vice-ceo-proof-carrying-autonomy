...
class HttpRouteSmokeTests(unittest.TestCase):
    ...
    def test_visual_demo_is_read_only_and_browser_ready(self) -> None:
        ...
        self.assertIn("View reply receipt", response.text)
        self.assertIn("View follow-up receipt", response.text)
        self.assertIn("Ready when enabled", response.text)
        self.assertIn("Needs you", response.text)
        self.assertIn("Stops on reply", response.text)
        self.assertIn("Delivery is your choice.", response.text)
